...
run_recon_passive() {
    ...
    if [ -f "$HOME/.config/tokens.txt" ]; then TOKENS_FILE="$HOME/.config/tokens.txt"; fi
    
    # Si no hay tokens.txt, intentar extraer de provider-config.yaml
    if [ -z "$TOKENS_FILE" ]; then
        for config_path in "$HOME/.config/subfinder/provider-config.yaml" \
                           "$HOME/.config/nuclei/provider-config.yaml" \
...
run_recon_active() {
    ...
    if command -v katana >/dev/null; then KATANA_BIN="katana"; else KATANA_BIN="$HOME/go/bin/katana"; fi
