...
if [ -f "/config/provider-config.yaml" ]; then
    ...
    cp "/config/provider-config.yaml" "$HOME/.config/subfinder/provider-config.yaml" 2>/dev/null || true
    cp "/config/provider-config.yaml" "$HOME/.config/nuclei/provider-config.yaml" 2>/dev/null || true
fi
